...
# 导入各个界面的Controller
from Controller_check import Controller_check
from Controller_cloudDetect import Controller_cloudDetect
from Controller_home import Controller_home
from Controller_select import Controller_select
from Controller_setting import Controller_setting
...
class Window(FramelessWindow):
    ...
    def load_designer_ui(self):
        ...
        # 索引1：第二个页面
        self.check_page = self.designer_ui.stackedWidget.widget(1)
        # 索引2：第三个页面
        self.cloudDetect_page = self.designer_ui.stackedWidget.widget(2)
        # 索引3：第四个页面
        self.select_page = self.designer_ui.stackedWidget.widget(3)
        # 索引4：第五个页面
        self.setting_page = self.designer_ui.stackedWidget.widget(4)

        # 用包装类处理页面，使其适配Fluent导航系统
        self.home_interface = DesignerWidgetWrapper(self.home_page, self)
        self.check_interface = DesignerWidgetWrapper(self.check_page, self)
        self.cloudDetect_interface = DesignerWidgetWrapper(self.cloudDetect_page, self)
        self.select_interface = DesignerWidgetWrapper(self.select_page, self)
        self.setting_interface = DesignerWidgetWrapper(self.setting_page, self)

        # 初始化各个Controller
        Controller_home(self.designer_ui)
        Controller_check(self.designer_ui)
        Controller_cloudDetect(self.designer_ui)
        Controller_select(self.designer_ui)
        Controller_setting(self.designer_ui)

        # self.designer_ui.tableWidget_1_allInfo
    ...
    def initNavigation(self):
        ...
        # 添加影像检查页（使用Designer的第二个页面）
        self.addSubInterface(
            self.check_interface,  # 对应的界面
            FIF.SEARCH_MIRROR,  # 图标
            '影像检查'  # 显示文本
        )

        # 添加云检测页
        self.addSubInterface(
            self.cloudDetect_interface,
            FIF.CLOUD,
            '云检测'
        )
    ...
    def initWindow(self):
        ...
        # 设置窗口标题
        self.setWindowTitle('遥感影像质量筛选与优化系统')
    ...
    def showMessageBox(self):
        ...
        w = MessageBox(
            '查看帮助文档💻',
            '若通过帮助文档无法解答您的问题，请及时反馈给我们📃。',
            self
        )
        w.yesButton.setText('帮助文档')
